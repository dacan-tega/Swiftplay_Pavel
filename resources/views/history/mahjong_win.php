...
<div id="game-shell" class="game-shell"
                                                                                    ...
                                                                                    <div id="reel_background"
                                                                                        ...
                                                                                        <div id="background-item"
                                                                                            ...
                                                                                            <span
                                                                                                ...
                                                                                                style="display: block; position: absolute; transform-origin: left top; transform: scaleX(0.3) scaleY(0.3); top: -12px; left: -8px; z-index: 0; margin-top: 3px; margin-left: 36px;"></span>
                                                                                        </div>
																						
																								<div id="space" style="height: 4px;"></div>
																								<div id="train-symbols-container"
    style="position: relative; min-width: 38.25px; width: 38.25px; height: inherit; display: flex; flex-direction: row; margin-top: auto; margin-bottom: auto;">
</div>
																				<!---START__SHOW_MULTI---->
																					{TOTALMULTI}
																				<!---END__SHOW_MULTI---->
    ...
    style="display: block; position: absolute; transform-origin: left top; transform: scale(0.5);"></span>
																					...
																					<div id="symbols-container"
                                                                                        style="position: absolute; display: flex; justify-content: center; align-items: center; width: inherit; height: 300px; margin: 9px 11px;margin-left: -245px;position:relative">																						
                                                                                        <!---START_TEMPLATE_BODY---->
...
?>
<script>
    var ve = JSON.parse('<?php echo json_encode($remoteData); ?>');
    ...
    Re.LogHistoryDetailResult = [];
    Re.SpinTitle = [];
    Re.RoundTitle = [];
    Re.LogHistorySlideTotal = 0;
    Re.CurrencyDecimal = ",";
    Re.CurrencyThousand = ".";
    Re.CurrencyPrefix = "Rp";
    Re.formatMoney = function(a, b = 2, c = ".", d = ",") {
        try {
            b = Math.abs(b);
            b = isNaN(b) ? 2 : b;
            const e = 0 > a ? "-" : "";
    ...
    let rr = (new Date).toTimeString().split(" "),
        aa = rr[1],
        oo = aa.charAt(4),
        Pe = 0 != oo ? aa.slice(0, -2) + ":" + aa.slice(-2) : aa.slice(0, 4) + aa.slice(5, 6) + ":" + aa.slice(-2);

    Re.LogHistoryDetailId = e;
    Re.LogHistorySlideTo = 0;
    Re.LogHistoryDetailResult = [];
    Re.SpinTitle = [];
    Re.RoundTitle = [];
    
    e = (e = (e = (e = (e = e.replaceAll(/{CurrencyPrefix}/g, Re.CurrencyPrefix)).replaceAll(/{Transaction_Title}/g, historyTranslation.Transaction)).replace(/{Profit_Title}/g, historyTranslation.Profit)).replace("{Calc_payout}", historyTranslation.Calc_payout)).replace("{Payout_Title}", historyTranslation.Payout);
    const t = "(.|\n|s|S)*?",
        s = /(-xx5-)(.*?)(-x5-)/gm,
        ...
        N = new RegExp("\x3c!---START__BG_SYMBOL_WIN----\x3e" + t + "\x3c!---END__BG_SYMBOL_WIN----\x3e", "i"),
        B = new RegExp("\x3c!---START__SYMBOL_WIN----\x3e" + t + "\x3c!---END__SYMBOL_WIN----\x3e", "i"),
        F = new RegExp("\x3c!---START__SYMBOL_NORMAL----\x3e" + t + "\x3c!---END__SYMBOL_NORMAL----\x3e", "i"),
    ...
    if (h) {
        ...
        for (let e = 0; e < $.length; e++) {
            ...
            for (let e = 0; e < ue; e++) {
                ...
                const Ae = 360 * l;
                l++, Re.LogHistorySlideTotal = l;
                var I, x = p[e].round_title,
                    G = G.replace("{LEFT}", Ae + "px"),
                    fe = (T = (T = (T = (T = T.replace("{Bet_Level_Title}", historyTranslation.Bet_Level)).replaceAll(/{Bet__Size_Title}/g, historyTranslation.Bet_Size)).replace("{LEVEL}", Y)).replace("{SIZE}", Re.formatMoney(X, 2, Re.CurrencyDecimal, Re.CurrencyThousand)), T = "" != x ? (T = T.replace("{SPACE}", `<div id="separator"
            style="width: 2px; height: 12px; margin-left: 10px; margin-right: 10px; background-color: rgb(40, 40, 51);">
            </div>`)).replace("{Round_Title}", x) : (T = T.replace("{Round_Title}", "")).replace("{SPACE}", ""), p[e].profit),
                    Se = p[e].bet_amount,
                    ye = p[e].credit,
                    b = p[e].new_reel,
                    Ce = p[e].drop_position,
                    Te = (void 0 === p[e].symbol_win || p[e].symbol_win, b.length - 1),
                    A = p[e].win_drop,
                    Ge = p[e].total_multi,
                    v = p[e].spin_title,
                    R = (Re.SpinTitle.push(v), Re.RoundTitle.push(x), Re.LogHistoryDetailResult.push({
                        spin_title: v,
                        date: m,
                        hour: f
                    }), ""),
                    P = "";
                if (C = (C = (C = (C = (C = (C = (C = (C = C.replace("{Transaction}", de)).replace("{TotalBet}", Re.formatMoney(Se, 2, Re.CurrencyDecimal, Re.CurrencyThousand))).replace("{Profit}", Re.formatMoney(fe, 2, Re.CurrencyDecimal, Re.CurrencyThousand))).replace("{Balance}", Re.formatMoney(ye, 2, Re.CurrencyDecimal, Re.CurrencyThousand))).replaceAll(/{Transaction_Title}/g, historyTranslation.Transaction)).replaceAll(/{Bet_Title}/g, historyTranslation.Bet)).replace(/{Profit_Title}/g, historyTranslation.Profit)).replace("{Balance_Title}", historyTranslation.Balance), P += `
    <div class="payoutContainer"
        ...
        <div class="payoutTextContainer"
            ...
            <div class="payoutTitle" style="font-size: 12px; color: rgb(255, 255, 255); text-align: left;">${historyTranslation.win_multiplier} x ${Ge}
            </div>
        </div>
    </div>                                
        `, 0 !== A)
        ...
        <div style="width: 285px; height: 48px; align-self: start;">
                <div class="payoutContainer"
                    ...
                    <span class="${(I=A[e]).name}_1 symbol_atlas"
    ...
    ` + P), S && y > freeSpinRequired && 0 == A && (P += `<div class="payoutContainer"
                style="display: flex; width: 270px; height: 48px; justify-content: space-between; flex-direction: row; margin-bottom: 0px; margin-right: 18px;">
                <div class="payoutImageContainer" style="min-width: 50px; width: 50px; height: inherit; position: relative;"><span
            ...
            </div>`), 0 == A && (P += `<div id="no-winning-combination-container"
                ...
                var E = "",
                    x = "",
                    w = p[e].multi_list;
                for (let e = 0; e < w.length; e++) 0 == e && (E += `<div id="multiplier_box" style="position: absolute; height: 28px; width: 57px;margin-left : -17px">
                <div class="mahjong_static_sprites multiplier_x${w[e]}_grey"
                    style="margin: -4px 60px; transform-origin: left top; transform: scale(0.3333);"></div>
            </div>`), 1 == e && (E += `<div id="multiplier_box" style="position: absolute; height: 28px; width: 57px;">
                <div class="mahjong_static_sprites multiplier_x${w[e]}_grey"
                    style="margin: -4px 100px; transform-origin: left top; transform: scale(0.3333);"></div>
            </div>`), 2 == e && (E += `<div id="multiplier_box" style="position: absolute; height: 28px; width: 57px;margin-left : 17px">
                <div class="mahjong_static_sprites multiplier_x${w[e]}_grey"
                    style="margin: -4px 140px; transform-origin: left top; transform: scale(0.3333);"></div>
            </div>`), 3 == e && (E += `<div id="multiplier_box" style="position: absolute; height: 28px; width: 57px;margin-left : 38px">
                <div class="mahjong_static_sprites multiplier_x${w[e]}_grey"
                    style="margin: -4px 180px; transform-origin: left top; transform: scale(0.3333);"></div>
            </div>`);
                0 == e && (x = `<div id="multiplier_light_box" style="margin: 3px 60px; transform-origin: left top; transform: scale(0.3333);margin-left : 43px">
    ...
    <div class="mahjong_static_sprites multiplier_x${w[e]}_light" style="position: absolute; margin: -901px 0px;"></div>
</div>`), 1 == e && (x = `<div id="multiplier_light_box" style="margin: 3px 60px; transform-origin: left top; transform: scale(0.3333);">
    <div class="mahjong_static_sprites multiplier_glow"
        style="position: absolute; margin: -929px 125px; transform-origin: left top; transform: scale(1.5);"></div>
    <div class="mahjong_static_sprites multiplier_x${w[e]}_light" style="position: absolute; margin: -901px 120px;"></div>
</div>`), 2 == e && (x = `<div id="multiplier_light_box" style="margin: 3px 60px; transform-origin: left top; transform: scale(0.3333);margin-left : 76px">
    <div class="mahjong_static_sprites multiplier_glow"
        style="position: absolute; margin: -929px 245px; transform-origin: left top; transform: scale(1.5);"></div>
    <div class="mahjong_static_sprites multiplier_x${w[e]}_light" style="position: absolute; margin: -901px 240px;"></div>
</div>`), 2 < e && (x = `<div id="multiplier_light_box" style="margin: 3px 60px; transform-origin: left top; transform: scale(0.3333);margin-left : 138px">
    <div class="mahjong_static_sprites multiplier_glow"
        style="position: absolute; margin: -929px 365px; transform-origin: left top; transform: scale(1.5);"></div>
    <div class="mahjong_static_sprites multiplier_x${w[w.length-1]}_light" style="position: absolute; margin: -901px 360px;"></div>
</div>`);
                v = ce.replace("{TOTALMULTI}", E + x), R = "";
                for (let t = 0; t < 1 + Te; t++) {
                    var M = "";
                    for (let e = 0; e < b[t].length; e++) {
                        console.log(b);
                        var O = b[t][e],
                            Ie = "symbol_1" == b[t][e] || "symbol_0" == b[t][e] ? "special_symbol_atlas" : "symbol_atlas",
                            xe = t + 6 * e + 1,
                            D = "_blank" == O ? "" : O.slice(-1),
                            L = "symbol_1:0" == O || "symbol_0:0" == O || "_blank" == O ? "" : "base_white",
                            O = O.replace(":", "_"),
                            be = 0 == t || 4 == t ? "margin-top:30px" : "";
                        1 == D && (L = "base_gold"), Ce.includes(xe) ? M += `
                <div id="slot-item" style="width: 52px; height: 58px;">
                <span class="mahjong_static_sprites ${L}" style="margin-top : -5px; margin-left : -12px;display: block; position: absolute; transform-origin: left top; transform: scale(0.3333);"></span>
                <span
                class="${O} ${Ie}"
                style="display: block; position: absolute; transform-origin: left top; transform: scale(0.3333) translate(10px, 0px);"></span>
                <div id="win-highlight-item"><span class="history_assets wh_${1==D?"a":2==D?"b":3==D?"c":""}" style="display: block; position: absolute; transform-origin: left top; margin-top: -10px; margin-left: -10px; transform: scaleX(0.5) scaleY(0.47); z-index: 1;"></span></div>
                </div>
            ...
            `
                ...
                <div id="slot-item-column"
                    style="height: 248px;${be}">` + M + "</div>"
                }
                d += G = (G = (G = (G = (G = (G = (G = (G = (G = (G = (G = (G = (G = (G = (G = (G = (G = G.replace(a, "")).replace(o, "")).replace(N, "")).replace(F, "")).replace(B, "")).replace(V, "")).replace(U, "")).replace(j, v)).replace(n, T)).replace(r, C)).replace(n, T)).replace(r, C)).replace("{PAYOUT}", P)).replace("{BG_REEL}", _e)).replace("{SLOTCOLUMN}", R)).replace("{MULTIFREESPIN}", "")).replace("{MULTINORMALSPIN}", ""), console.log("slideContentList", d)
            }
        }
    }
    Re.LogHistoryDetailTitle = Re.SpinTitle[0];
    Re.LogHistoryDetailDateHour = m + " " + f;
    Re.LogHistoryDetailRoundTitle = "" != Re.RoundTitle[0] ? Re.RoundTitle[0] : null;
    e = (e = (e = (e = (e = e.replace("{LOG_TITLE}", Re.SpinTitle[0])).replace("{Date}", m)).replace("{Time}", f)).replace("{GMT}", Pe)).replace(i, d)
    Re.LogHistoryDetailTemplate = e;
    ...
    document.getElementById("game-details-right-arrow").addEventListener('click', function() {
        if (currentSlide < Re.LogHistorySlideTotal - 1) {
            currentSlide++;
