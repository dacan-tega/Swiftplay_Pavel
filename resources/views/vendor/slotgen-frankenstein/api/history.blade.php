...
<style>
    ...
    :root {
        ...
        --reel-height: calc(var(--reelslot-sh, 3)*var(--slot-item-height));
    ...
    .bg {
        ...
        background-size: 243px 106px;
        background-image: url(/uploads/games/en/frankenstein-1736929261/symbols/bg.png);
    }

    .Table__Row--details.Table__Row--expanded {
        --body-scale: 1.5;
...
<body class="Body--scaled">
    <div id="root">
        <div id="content" class="App App__Jurisdiction--UK" style="width: 100%; height: 100%;">
            <h3 class="GameTitle">Spooky Frankenstein</h3>
    ...
    <script>
        $(document).ready(function() {
                                    ...
                                    $.ajax({
                                        ...
                                        <div class="vs20olympgate">`;
                                            ...
                                            for (let d = 0; d < item.result_json.length; d++) {
                                                ...
                                                var arrValueMulti = item.result_json[d]['value_multi_arr'];
                                                var arrNumberMulti = item.result_json[d]['number_multi_arr'];
                                                var totalBetAlive = item.result_json[0]['total_bet_alive'];

                                                var multiResult = "";
                                                for (let i = 0; i < arrValueMulti.length; i++) {
                                                    var valueMulti = arrValueMulti[i] * totalBetAlive;
                                                    var numberMulti = arrNumberMulti[i] > 1 ? `x${arrNumberMulti[i]}` : "";
                                                    multiResult = multiResult + `<div class="box">${valueMulti} <span class="multiplier">${numberMulti}</span></div>`;
                                                }
                                                action = `
                                                ...
                                                //     var winDrop = item.result_json[d]['win_drop'][i]['active_icon'];
                                                // }
                                                ...
                                                winTotal = parseFloat(winTotal + totalWin);
                                                var freeMode = item.result_json[d]['free_mode'];
                                                // freeMode = true;
                                                var countStartScatter = freeMode ? 0 : 2;
                                                var numberReelSlot = freeMode ? 1 : "";
                                                for (let i = 0; i < lastReel.length; i++) {
                                                    var symbolreel = "";
                                                    var reelPosition = "";
                                                    for (let j = countStartScatter; j < lastReel[i].length; j++) {
                                                        // var check = i + j * lastReel[i].length;
                                                        var check = i * lastReel[i].length + (lastReel[i].length - j) - 1;
                                                        let symName = lastReel[i][j];
                                                        if (symName.includes(":")) {
                                                            let arrExplode = symName.split(":");
                                                            symName = arrExplode[0];
                                                        }
                                                        if (lastReel[i][j].search("x") > 0) {
                                                            ...
                                                            // winDrop = [];
                                                            if (winDrop.includes(check)) {
                                                                symbolreel = symbolreel + `<div class="ReelsSlot__Item ReelsSlot__Item--position-${check}">
                                                                <div class="Animation Animation__CrossFade ">
                                                                <div class="Animation__CrossFadeItem Animation__CrossFadeItem--0"><img src="/uploads/games/${gameName}/symbols/${symName}.png" srcset="" alt="10" class="ReelsSlot__ItemImg"></div>
                                                                ...
                                                                <div class="ReelsSlot__Item ReelsSlot__Item--position-${check} ReelsSlot__--${symName}"><img src="/uploads/games/${gameName}/symbols/${symName}.png" srcset="" alt="${symName}" class="ReelsSlot__ItemImg"></div>
                                                                 `;
                                                            }
                                                            // exitingExpand.includes(id)
                                                        }
                                                    }
                                                    reel = reel + `<div class="ReelsSlot__Reel${numberReelSlot}">` + symbolreel + `</div>`;
                                                    ...
                                                    for (let j = countStartScatter; j < lastReel[i].length; j++) {
                                                        ...
                                                        if (lastReel[i][j].search("x") > 0) {
                                                            ...
                                                            if (winDrop.includes(check + 1)) {
                                                                ...
                                                                <div class="ReelsSlot__Item ReelsSlot__Item--position-${check} ReelsSlot__Item--win ReelsSlot__--${symName}"><img src="/uploads/games/${gameName}/symbols/${symName}.png" srcset="" alt="${symName}" class="ReelsSlot__ItemImg"></div>`
                                                            } else {
                                                                winPosition = winPosition + `
                                                                <div class="ReelsSlot__Item ReelsSlot__Item--position-${check} ReelsSlot__--${symName}"><img src="/uploads/games/${gameName}/symbols/${symName}.png" srcset="" alt="${symName}" class="ReelsSlot__ItemImg"></div>
                                                                 `;
            ...
            function resizeRoot() {
                var scale = 1;
                if ($('.Table__Row--details.Table__Row--expanded').length > 0) {
                    scale = Math.min(window.innerWidth / 440, window.innerHeight / 270);
                } else {
                    scale = Math.min(window.innerWidth / 360, window.innerHeight / 270);
                }
                scale = scale > 1 ? 1 : scale;
                document.documentElement.style.setProperty('--body-scale', scale);
            }
            ...
            // Trigger onResize function when the window is resized
