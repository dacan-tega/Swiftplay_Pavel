...
class Core
{
     ...
     * @return mixed
     */
    public static function generateGameHistory($user, $sesionId , $transaction , $type, $amount, $bet, $balance, $profit, $nameGame, $gameId, bool $changeBonus = false, $provider,$agenId,$wallet)
    // public static function generateGameHistory(User $user, $type, $amount, $bet, $balance, $profit, $nameGame, $gameId, bool $changeBonus = false, $provider,$agenId)
    {
        ...
        if ($type == 'win' && !empty($user->inviter)) {
            ...
            if (!empty($affiliate)) {
                ...
                if (!empty($affHistoryRevshare)) {
                    ...
                    $wallet->decrement('refer_rewards', $commissionPay);
        ...
        /// devolver dinheiro afiliado
        if ($type == 'tie' && !empty($user->inviter)) {
            if (!empty($affiliate)) {
                $affHistoryRevshare = AffiliateHistory::where('user_id', $user->id)
                    ->where('commission_type', 'revshare')
                    //->where('deposited', 1)
        ...
        $userCheck = User::where("id", $user->user_name)->get();
        $userCheck = false;
        if ($agenId == -1) {
            $userName = empty($user->user_name) ? $user->name : $user->name;
            $findUser = User::where("name", $userName)->get();
            if (!empty($findUser->all())) {
                $userCheck = true;
            }
        }
        $userId = $userCheck  ? $user->id : 1 ;
        return Order::create([
            'user_id' => $userId,
            // 'user_id' => 1,
            'session_id' => $sesionId,
    ...
    public static function upload($file)
    {
        ...
        $size       = $file->getSize();
        $path       = Storage::disk('public')->putFileAs('uploads', $file, $nameFile);
...
/*
    public function sendCurl($data, $url)
    {
        ...
        if ($httpcode == 403) {
            return "Unable to access API 403";
...
public function sendCurl($data, $url)
    {
        $interceptAPICalls = env('INTERCEPT_API_CALL', false);
        ...
        if ($interceptAPICalls) {
            try {
                ...
                $rabbitmqHelper->openConnection();

                $result = $rabbitmqHelper->publishCallMessage(json_encode($dataToSend));
            ...
            $act = isset($data['action']) ? $data['action'] : "";
            $ch = curl_init();
            $payload = json_encode($data);
            $headers = [
                'Accept: application/json',
