...
class GameController extends Controller
{
    ...
    public function index(Request $request, $slug)
    {
        ...
        if(!empty($game)) {
            if(auth()->check()) {
                ...
                $token = hash('sha256','md5 casino'.md5(auth()->user()->email.'-'.time()));
        ...
        return back()->with('error', __('admin.need_play'));
    }
    ...
    public function getListGame(Request $request)
    {
        ...
        return view('web.game.list', compact(['games', 'search', 'tab','categories']));
    }
    ...
    private function listProvider($request)
    {
        ...
        return $query_games->paginate(18)->appends(request()->query());
    }
