...
class AffiliateController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('panel.affiliates.index', compact('token', 'indications', 'histories','categories'));
    }
    ...
    public function joinAffiliate(Request $request)
    {
        ...
        \Mail::send('emails.join-affiliate', ['email' => $request->get('email')], function ($message) use($request) {
            ...
            $message->to(env('MAIL_FROM_ADDRESS'), __('admin.AFFILIATE_APPLICATION'))
                    ->subject(env('APP_NAME'));
        });

        return back()->with('success', __('admin.email_sent_successfully'));
    }
