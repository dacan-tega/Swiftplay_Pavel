...
namespace Slotgen\SlotgenSweetBonanza\Http\Controllers\Api;
...
use Slotgen\SlotgenSweetBonanza\Helpers\Common;
use Slotgen\SlotgenSweetBonanza\Http\Controllers\AppBaseController;
use Slotgen\SlotgenSweetBonanza\Models\SweetBonanzaPlayer;
use Slotgen\SlotgenSweetBonanza\Models\SweetBonanzaSpinLogs;
use Slotgen\SlotgenSweetBonanza\Models\SlotgenSweetBonanzaConfig;
use Slotgen\SlotgenSweetBonanza\SlotgenSweetBonanza;
...
class GameController extends AppBaseController
{
    public static function launchGameApi()
    {
        ...
        $gameFile = null;
        $gamePrivateFolder = storage_path('app/private/sweet_bonanza');
        ...
        $launchData = SlotgenSweetBonanza::checkPlayer($player);
        $launchGameRes = SlotgenSweetBonanza::LaunchGame($launchData);
        if ($launchGameRes['success']) {
            $resData = SlotgenSweetBonanza::LaunchGameRes($launchGameRes);
            if ($resData['success']) {
    ...
    public function launchGame(Request $request)
    {
        ...
        if ($checkLaunchGame) {
            $player = SweetBonanzaPlayer::where('player_uuid', $userName)->first();
            if ($player) {
                $launchData = [
                    'uuid' => $player->uuid,
            ...
            if ($launchGameRes['success']) {
                ...
                $resLaunch = SlotgenSweetBonanza::LaunchGameRes($launchGameRes);
                $resData = [
    ...
    public function history(Request $request)
    {
        ...
        $api_url = route('api.sweetbonanza.v1.root');
        $request = $request->all();
        $token = $request['token'];

        return view('slotgen-sweetbonanza::api.history', compact('token', 'api_url', 'gameName', 'history'));
    }

    public function GameAction(Request $request)
    {
        $startTime = microtime(true) * 1000;
        $adjustRatio = (object) SlotgenSweetBonanzaConfig::first();
        //###############
        ...
        $path = storage_path('app/private/sweet_bonanza');
        ...
        // $userNameApi = "";
        $sessionPlayer = SweetBonanzaPlayer::where('uuid', $token)->first();
        ...
        if ($sessionPlayer) {
            ...
            if ($act === 'session' || $act === 'spin' || $act === 'load_session' || $act === 'buy') {
                ...
                if ($USE_SEAMLESS) {
                    ...
                    $userPlayer = SweetBonanzaPlayer::where('player_uuid', $userNameAgent)->first();
            ...
            if ($act === 'icons') {
                $session = SweetBonanzaPlayer::where('uuid', $token)->first();
                // var_dump(($gameRule));
                if ($session) {
                    if ($gameData) {
                    ...
                    if ($gameData && $gameRule && $spinData) {
                        ...
                        if ($betamount && $bet_level) {
                            ...
                            if ($wallet >= $totalBet && $totalBet <= $MAX_BET && $totalBet >= $MIN_BET || $freeMode) {
                                ...
                                // $fileName = "slotgen_win_70_data.txt";
                                // $lineIndex = 2;
                                ...
                                if ($pull) {
                                    ...
                                    $spinLogs = new SweetBonanzaSpinLogs;
                                    $data = [
                                    ...
                                    $lastid = SweetBonanzaSpinLogs::latest()->first()->uuid;
                                    ...
                                    if ($parentId != 0 && $freeMode) {
                                        $recordFree = SweetBonanzaSpinLogs::where('uuid', $parentId)->first();
                                        ...
                                        SweetBonanzaSpinLogs::where('uuid', $parentId)->update(['win_amount' => $winAmount, 'balance' => $wallet]);
                                    }
            ...
            if ($act === 'histories') {
                ...
                if ($sessionPlayer) {
                    ...
                    $totalBet = SweetBonanzaSpinLogs::where($search)->whereBetween('created_at', [$from, $to])->sum('total_bet');
                    $totalWin = SweetBonanzaSpinLogs::where($search)->whereBetween('created_at', [$from, $to])->sum('win_amount');
                    $totalProfit = $totalWin - $totalBet;
                    $limit = 12;
                    $page = $page ? $page : 1;
                    $recorsPerPage = ($page - 1) * $limit;
                    $paginate = SweetBonanzaSpinLogs::where($search)->whereBetween('created_at', [$from, $to])
            ...
            if ($act === 'history_detail') {
                ...
                if ($session) {
                    ...
                    $resultJson = [];
                    $totalMultiJson = [];
                    $history = SweetBonanzaSpinLogs::where('uuid', $uuid)->first();
                    if ($history) {
                        $historyChild = SweetBonanzaSpinLogs::where('parent_id', $uuid)->get();
    ...
    {
        $playerId = SweetBonanzaPlayer::where('player_uuid', $playerId)->first();
