...
namespace Slotgen\SlotgenSweetBonanza\Http\Controllers\Site;
...
class GameController extends AppBaseController
{
    ...
    public function rtpSimulate(Request $req)
    {
        ...
        // $player = auth()->user();
        // $playerId = $player->id;
        // $playerGame = SweetBonanzaPlayer::where("player_uuid",$playerId)->first();
        ...
        $ip = SlotgenSweetBonanza::getIp($req);
        $data = [
            'device_info' => $agent,
            'client_ip' => $ip,
        ];
        $launchData = SlotgenSweetBonanza::checkPlayer($player, $data);
        $launchGameRes = (object) SlotgenSweetBonanza::LaunchGame($launchData);
