...
class SlotgenSweetBonanza
{
    ...
    {
        // $api_url = route('web.sweetbonanza.site.launch');
        ...
        if (File::exists($gameFile)) {
            ...
            $logo_url = url('uploads/games/' . $gameFolder . '/sweetbonanza-logo.png');
        } else {
            $gameFolder = '';
    ...
    {
        $setting = (object) SlotgenSweetBonanzaConfig::first();
        ...
        $dataType = 'normal';
        SlotgenSweetBonanza::spinConfig($path, $dataType);
        ...
        if ($USE_SEAMLESS) {
            $user = SlotgenSweetBonanza::PlayerEntityId($userName);
        } else {
            $user = SlotgenSweetBonanza::PlayerEntityId($playerId);
        }
        ...
        if (! $user) {
            if ($gameName) {
                ...
                $playerData = new SweetBonanzaPlayer;
                $playerData->fill($data);
                $playerData->save();
                $user = SweetBonanzaPlayer::where('uuid', $playerData->uuid)->first();
                // dd($user);
                if (! $playerData) {
