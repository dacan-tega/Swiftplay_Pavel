...
if ($formsOnly) {
    safeUnlink(__DIR__ . '/src/SlotgenWildBanditoTheme.php');
    safeUnlink(__DIR__ . '/src/SlotgenWildBanditoPlugin.php');
    ...
    if ($isTheme) {
        safeUnlink(__DIR__ . '/src/SlotgenWildBanditoServiceProvider.php');
        safeUnlink(__DIR__ . '/src/SlotgenWildBanditoPlugin.php');
        safeUnlink(__DIR__ . '/src/SlotgenWildBandito.php');
...
foreach ($files as $file) {
    replaceInFile($file, [
        ...
        'slotgen-wildbandito' => $packageSlug,
        'slotgen-wild_bandito_without_prefix' => $packageSlugWithoutPrefix,
        'SlotgenWildBandito' => $className,
        'slotgen-wildbandito' => $packageSlug,
        'migration_table_name' => titleSnake($packageSlug),
        'variable' => $variableName,
        'Wild Bandito plugin for Filament' => $description,
    ]);

    match (true) {
        str_contains($file, determineSeparator('src/SlotgenWildBandito.php')) => rename($file, determineSeparator('./src/' . $className . '.php')),
        str_contains($file, determineSeparator('src/SlotgenWildBanditoServiceProvider.php')) => rename($file, determineSeparator('./src/' . $className . 'ServiceProvider.php')),
        str_contains($file, determineSeparator('src/SlotgenWildBanditoTheme.php')) => rename($file, determineSeparator('./src/' . $className . (str_ends_with($className, 'Theme') ? '.php' : 'Theme.php'))),
        str_contains($file, determineSeparator('src/SlotgenWildBanditoPlugin.php')) => rename($file, determineSeparator('./src/' . $className . 'Plugin.php')),
        str_contains($file, determineSeparator('src/Facades/SlotgenWildBandito.php')) => rename($file, determineSeparator('./src/Facades/' . $className . '.php')),
        str_contains($file, determineSeparator('src/Commands/SlotgenWildBanditoCommand.php')) => rename($file, determineSeparator('./src/Commands/' . $className . 'Command.php')),
        str_contains($file, determineSeparator('src/Testing/TestsSlotgenWildBandito.php')) => rename($file, determineSeparator('./src/Testing/Tests' . $className . '.php')),
        str_contains($file, determineSeparator('database/migrations/create_slotgenwildbandito_table.php.stub')) => rename($file, determineSeparator('./database/migrations/create_' . titleSnake($packageSlugWithoutPrefix) . '_table.php.stub')),
        str_contains($file, determineSeparator('config/slotgen-wildbandito.php')) => rename($file, determineSeparator('./config/' . $packageSlugWithoutPrefix . '.php')),
        str_contains($file, determineSeparator('resources/lang/en/slotgen-wildbandito.php')) => rename($file, determineSeparator('./resources/lang/en/' . $packageSlugWithoutPrefix . '.php')),
...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i ' . basename(__FILE__) . ' | findstr /r /i /M /F:/ ":author :vendor :package Slotgen slotgen-wildbandito migration_table_name vendor_name vendor_slug author@domain.com"'));
}
...
function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('find ./* ./.github/* -name "vendor" -type d -prune \
     ...
     -o -type f -print0 | xargs -0 grep -E -r -l -i ":author|:vendor|:package|Slotgen|slotgen-wildbandito|migration_table_name|vendor_name|vendor_slug|author@domain.com"'));
}
