...
namespace Slotgen\SlotgenWildBandito\Http\Controllers\Api;
...
use Slotgen\SlotgenWildBandito\Helpers\Common;
use Slotgen\SlotgenWildBandito\Http\Controllers\AppBaseController;
use Slotgen\SlotgenWildBandito\Models\WildBanditoPlayer;
use Slotgen\SlotgenWildBandito\Models\WildBanditoSpinLogs;
use Slotgen\SlotgenWildBandito\Models\SlotgenWildBanditoConfig;
...
class GameController extends AppBaseController
{
    public function info(Request $request)
    {
        ...
        $api_url = route('api.wildbandito.v1.root');
        if ($act) {
            if ($act == 'payout') {
                return view('slotgen-wildbandito::api.payout', compact('gameName', 'gamePublicFolder'));
            } elseif ($act == 'gamerule') {
                return view('slotgen-wildbandito::api.game_rule', compact('gameName', 'gamePublicFolder'));
            } elseif ($act == 'histories') {
                return view('slotgen-wildbandito::api.history', compact('token', 'api_url', 'gameName', 'gamePublicFolder'));
            }
        } else {
            return 'Empty action';
        }
    ...
    public function historyDetail(Request $request)
    {
        ...
        $setting = (object) SlotgenWildBanditoConfig::first();
        $currency = $setting->currency;
        $token = $request['token'];
        $uuid = $request['uuid'];
        $language = isset($request['language']) ? $request['language'] : 'en';
        // $game_rule = File::get(__DIR__ . "/../../../../resources/games/config/game.json");
        ...
        return view('slotgen-wildbandito::api.history_detail', compact('token', 'uuid', 'api_url', 'gameName', 'gamePublicFolder', 'currency'));
    }
    ...
    public function launchGame(Request $request)
    {
        ...
        if ($checkLaunchGame) {
            $player = WildBanditoPlayer::where('player_uuid', $userName)->first();
            if ($player) {
                $launchData = [
                    'uuid' => $player->uuid,
            ...
            if ($launchGameRes['success']) {
                ...
                $resLaunch = SlotgenWildBandito::LaunchGameRes($launchGameRes);
                $resData = [
    ...
    public function history(Request $request)
    {
        ...
        return view('slotgen-wildbandito::api.history', compact('token', 'api_url', 'gameName', 'history'));
    }

    public function GameAction(Request $request)
    {
        $adjustRatio = (object) SlotgenWildBanditoConfig::first();
        //###############
        ...
        $path = storage_path('app/private/wild_bandito');
        ...
        // $userNameApi = "";
        $sessionPlayer = WildBanditoPlayer::where('uuid', $token)->first();
        ...
        if ($sessionPlayer) {
            ...
            if ($act === 'session' || $act === 'spin' || $act === 'load_session' || $act === 'buy') {
                ...
                if ($USE_SEAMLESS) {
                    ...
                    $userPlayer = WildBanditoPlayer::where('player_uuid', $userNameAgent)->first();
            ...
            if ($act === 'load_session') {
                ...
                if ($sessionPlayer) {
                    ...
                    $translate = $langInfo;
                    $sufix = $ssData->currency_suffix == null ? '' : $ssData->currency_suffix;
                    $resData = (object) [
                        'user_name' => $userName,
            ...
            if ($act === 'icons') {
                $session = WildBanditoPlayer::where('uuid', $token)->first();
                // var_dump(($gameRule));
                if ($session) {
                    if ($gameData) {
                    ...
                    if ($gameData && $gameRule && $spinData) {
                        ...
                        if ($betamount && $bet_level) {
                            ...
                            if ($wallet >= $totalBet && $totalBet <= $MAX_BET && $totalBet >= $MIN_BET || $freeMode) {
                                ...
                                // $fileName = "slotgen_win_122_data.txt";
                                // $lineIndex = 2;
                                ...
                                if ($pull) {
                                    ...
                                    for ($i = 0; $i < count($resulJson); $i++) {
                                        ...
                                        $resulJson[$i]->bet_amount = $i == 0 ? number_format($totalBet, 2, '.', '') : 0;
                                        ...
                                        for ($j = 0; $j < count($resulJson[$i]->win_drop); $j++) {
                                            $resulJson[$i]->win_drop[$j]->win = number_format($resulJson[$i]->win_drop[$j]->win * $ajustRatio, 2, '.', '');
                                            $resulJson[$i]->win_drop[$j]->win_multi = number_format($resulJson[$i]->win_drop[$j]->win_multi * $ajustRatio, 2, '.', '');
                                            $resulJson[$i]->win_drop[$j]->win_total = number_format($resulJson[$i]->win_drop[$j]->win_total * $ajustRatio, 2, '.', '');
                                            $profitNew = $profitNew + $resulJson[$i]->win_drop[$j]->win_multi;
                                        }
                                        $resulJson[$i]->win_total = number_format($resulJson[$i]->win_total * $ajustRatio, 2, '.', '');
                                        $resulJson[$i]->win_multi = number_format($resulJson[$i]->win_multi * $ajustRatio, 2, '.', '');
                                        $resulJson[$i]->profit = $profitNew - $resulJson[$i]->bet_amount;
                                        $walletNew = $walletNew + $profitNew;
                                        $resulJson[$i]->credit_drop = number_format($walletNew, 2, '.', '');
                                        $resulJson[$i]->credit =  $resulJson[$i]->credit_drop;
                                    }
                                    ...
                                    $spinLogs = new WildBanditoSpinLogs;
                                    $data = [
                                    ...
                                    $lastid = WildBanditoSpinLogs::latest()->first()->uuid;
                                    ...
                                    if ($parentId != 0 && $freeMode) {
                                        $recordFree = WildBanditoSpinLogs::where('uuid', $parentId)->first();
                                        ...
                                        WildBanditoSpinLogs::where('uuid', $parentId)->update(['win_amount' => $winAmount]);
                                    }
                                    ...
                                    $resData = [
                                        'result' => $totalResult,
                                        'win_amount' => $pull->win_amount,
                                        'win_multi' => $pull->win_multi,
                                        'bet_amount' => $totalBet,
                                        'credit' => number_format($wallet, 2, '.', ''),
                                        'free_mode' => $pull->free_mode,
                                        'freespin_win' => $pull->freespin_win,
                                        'free_num' => $pull->free_num,
                                        'total_multi' => $pull->total_multi,
                                        'free_more' => $pull->free_more,
                                        'freespin_require' => $pull->freespin_require,
                                        'multiply_step' => $pull->multiply_step,
                                        'freespin_more' => $pull->freespin_more,
            ...
            if ($act === 'histories') {
                ...
                if ($sessionPlayer) {
                    ...
                    $totalBet = WildBanditoSpinLogs::where($search)->whereBetween('created_at', [$from, $to])->sum('total_bet');
                    $totalWin = WildBanditoSpinLogs::where($search)->whereBetween('created_at', [$from, $to])->sum('win_amount');
                    $totalProfit = $totalWin - $totalBet;
                    $limit = 12;
                    $freeRequire = $gameData->freespin_require;
                    $page = $page ? $page : 1;
                    $recorsPerPage = ($page - 1) * $limit;
                    $paginate = WildBanditoSpinLogs::where($search)->whereBetween('created_at', [$from, $to])
            ...
            if ($act === 'history_detail') {
                ...
                if ($session) {
                    ...
                    $resultJson = [];
                    $history = WildBanditoSpinLogs::where('uuid', $uuid)->first();

                    if ($history) {
                        $history = $history->toArray();
                        $betSize = $history['betamount'];
                        $betLevel = (int) $history['credit_line'];
                        $historyChild = WildBanditoSpinLogs::where('parent_id', $uuid)->get()->toArray();
                        $spinTitleFirst = 'Normal Spin';
                        $spinDate = date('y/m/d', strtotime($history['created_at']));
                        $spinHour = date('H:i:s', strtotime($history['created_at']));
                        for ($i = 0; $i < count($history['result_json']); $i++) {
                            ...
                            $history['result_json'][$i]['spin_date'] = $spinDate;
                            $history['result_json'][$i]['spin_hour'] = $spinHour;
                        }
                        ...
                        for ($i = 0; $i < count($historyChild); $i++) {
                            ...
                            $totalFreeSpin = $historyChild[$i]['result_json'][$numberDrop]['total_freespin'];
                            $numberFreeSpin = $i + 1;
                            ...
                            for ($j = 0; $j < count($historyChild[$i]['result_json']); $j++) {
                                ...
                                $historyChild[$i]['result_json'][$j]['spin_date'] = $spinDateChild;
                                $historyChild[$i]['result_json'][$j]['spin_hour'] = $spinHourChild;
                            }
    ...
    {
        $playerId = WildBanditoPlayer::where('player_uuid', $playerId)->first();
