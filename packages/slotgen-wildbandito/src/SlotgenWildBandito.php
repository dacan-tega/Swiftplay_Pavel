...
class SlotgenWildBandito
{
    public static function getGame()
    {
        // $api_url = route('web.wildbandito.site.launch');
        ...
        if (File::exists($gameFile)) {
            ...
            $logo_url = url('uploads/games/' . $gameFolder . '/wildbandito-logo.png');
        } else {
            $gameFolder = '';
    ...
    public static function LaunchGame($launchData)
    {
        ...
        $dataType = 'normal';
        SlotgenWildBandito::spinConfig($path, $dataType);
        ...
        if ($USE_SEAMLESS) {
            $user = SlotgenWildBandito::PlayerEntityId($userName);
        } else {
            $user = SlotgenWildBandito::PlayerEntityId($playerId);
        }
        ...
        if (! $user) {
            if ($gameName) {
                ...
                $playerData = new WildBanditoPlayer;
                $playerData->fill($data);
                $playerData->save();
                $user = WildBanditoPlayer::where('uuid', $playerData->uuid)->first();
                // dd($user);
                if (! $playerData) {
