...
class SlotgenBigBassBonanza
{
    ...
    {
        // $api_url = route('web.bigbassbonanza.site.launch');
        ...
        if (File::exists($gameFile)) {
            ...
            $logo_url = url('uploads/games/' . $gameFolder . '/bigbassbonanza-logo.png');
        } else {
            $gameFolder = '';
    ...
    {
        $setting = (object) SlotgenBigBassBonanzaConfig::first();
        ...
        $dataType = 'normal';
        SlotgenBigBassBonanza::spinConfig($path, $dataType);
        ...
        if ($USE_SEAMLESS) {
            $user = SlotgenBigBassBonanza::PlayerEntityId($userName);
        } else {
            $user = SlotgenBigBassBonanza::PlayerEntityId($playerId);
        }
        ...
        if (! $user) {
            if ($gameName) {
                ...
                $playerData = new BigBassBonanzaPlayer;
                $playerData->fill($data);
                $playerData->save();
                $user = BigBassBonanzaPlayer::where('uuid', $playerData->uuid)->first();
                // dd($user);
                if (! $playerData) {
