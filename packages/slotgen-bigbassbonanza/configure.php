...
if ($formsOnly) {
    safeUnlink(__DIR__ . '/src/SlotgenBigBassBonanzaTheme.php');
    safeUnlink(__DIR__ . '/src/SlotgenBigBassBonanzaPlugin.php');
    ...
    if ($isTheme) {
        safeUnlink(__DIR__ . '/src/SlotgenBigBassBonanzaServiceProvider.php');
        safeUnlink(__DIR__ . '/src/SlotgenBigBassBonanzaPlugin.php');
        safeUnlink(__DIR__ . '/src/SlotgenBigBassBonanza.php');
...
foreach ($files as $file) {
    replaceInFile($file, [
        ...
        'slotgen-bigbassbonanza' => $packageSlug,
        'slotgen-big_bass_bonanza_without_prefix' => $packageSlugWithoutPrefix,
        'SlotgenBigBassBonanza' => $className,
        'slotgen-bigbassbonanza' => $packageSlug,
        'migration_table_name' => titleSnake($packageSlug),
        'variable' => $variableName,
        'Big Bass Bonanza plugin for Filament' => $description,
    ]);

    match (true) {
        str_contains($file, determineSeparator('src/SlotgenBigBassBonanza.php')) => rename($file, determineSeparator('./src/' . $className . '.php')),
        str_contains($file, determineSeparator('src/SlotgenBigBassBonanzaServiceProvider.php')) => rename($file, determineSeparator('./src/' . $className . 'ServiceProvider.php')),
        str_contains($file, determineSeparator('src/SlotgenBigBassBonanzaTheme.php')) => rename($file, determineSeparator('./src/' . $className . (str_ends_with($className, 'Theme') ? '.php' : 'Theme.php'))),
        str_contains($file, determineSeparator('src/SlotgenBigBassBonanzaPlugin.php')) => rename($file, determineSeparator('./src/' . $className . 'Plugin.php')),
        str_contains($file, determineSeparator('src/Facades/SlotgenBigBassBonanza.php')) => rename($file, determineSeparator('./src/Facades/' . $className . '.php')),
        str_contains($file, determineSeparator('src/Commands/SlotgenBigBassBonanzaCommand.php')) => rename($file, determineSeparator('./src/Commands/' . $className . 'Command.php')),
        str_contains($file, determineSeparator('src/Testing/TestsSlotgenBigBassBonanza.php')) => rename($file, determineSeparator('./src/Testing/Tests' . $className . '.php')),
        str_contains($file, determineSeparator('database/migrations/create_slotgenbigbassbonanza_table.php.stub')) => rename($file, determineSeparator('./database/migrations/create_' . titleSnake($packageSlugWithoutPrefix) . '_table.php.stub')),
        str_contains($file, determineSeparator('config/slotgen-bigbassbonanza.php')) => rename($file, determineSeparator('./config/' . $packageSlugWithoutPrefix . '.php')),
        str_contains($file, determineSeparator('resources/lang/en/slotgen-bigbassbonanza.php')) => rename($file, determineSeparator('./resources/lang/en/' . $packageSlugWithoutPrefix . '.php')),
...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i ' . basename(__FILE__) . ' | findstr /r /i /M /F:/ ":author :vendor :package Slotgen slotgen-bigbassbonanza migration_table_name vendor_name vendor_slug author@domain.com"'));
}
...
function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('find ./* ./.github/* -name "vendor" -type d -prune \
     ...
     -o -type f -print0 | xargs -0 grep -E -r -l -i ":author|:vendor|:package|Slotgen|slotgen-bigbassbonanza|migration_table_name|vendor_name|vendor_slug|author@domain.com"'));
}
