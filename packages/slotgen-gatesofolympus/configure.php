...
if ($formsOnly) {
    safeUnlink(__DIR__ . '/src/SlotgenGatesOfOlympusTheme.php');
    safeUnlink(__DIR__ . '/src/SlotgenGatesOfOlympusPlugin.php');
    ...
    if ($isTheme) {
        safeUnlink(__DIR__ . '/src/SlotgenGatesOfOlympusServiceProvider.php');
        safeUnlink(__DIR__ . '/src/SlotgenGatesOfOlympusPlugin.php');
        safeUnlink(__DIR__ . '/src/SlotgenGatesOfOlympus.php');
...
foreach ($files as $file) {
    replaceInFile($file, [
        ...
        'slotgen-gatesofolympus' => $packageSlug,
        'slotgen-gates_of_olympus_without_prefix' => $packageSlugWithoutPrefix,
        'SlotgenGatesOfOlympus' => $className,
        'slotgen-gatesofolympus' => $packageSlug,
        'migration_table_name' => titleSnake($packageSlug),
        'variable' => $variableName,
        'Gates Of Olympus plugin for Filament' => $description,
    ]);

    match (true) {
        str_contains($file, determineSeparator('src/SlotgenGatesOfOlympus.php')) => rename($file, determineSeparator('./src/' . $className . '.php')),
        str_contains($file, determineSeparator('src/SlotgenGatesOfOlympusServiceProvider.php')) => rename($file, determineSeparator('./src/' . $className . 'ServiceProvider.php')),
        str_contains($file, determineSeparator('src/SlotgenGatesOfOlympusTheme.php')) => rename($file, determineSeparator('./src/' . $className . (str_ends_with($className, 'Theme') ? '.php' : 'Theme.php'))),
        str_contains($file, determineSeparator('src/SlotgenGatesOfOlympusPlugin.php')) => rename($file, determineSeparator('./src/' . $className . 'Plugin.php')),
        str_contains($file, determineSeparator('src/Facades/SlotgenGatesOfOlympus.php')) => rename($file, determineSeparator('./src/Facades/' . $className . '.php')),
        str_contains($file, determineSeparator('src/Commands/SlotgenGatesOfOlympusCommand.php')) => rename($file, determineSeparator('./src/Commands/' . $className . 'Command.php')),
        str_contains($file, determineSeparator('src/Testing/TestsSlotgenGatesOfOlympus.php')) => rename($file, determineSeparator('./src/Testing/Tests' . $className . '.php')),
        str_contains($file, determineSeparator('database/migrations/create_slotgengatesofolympus_table.php.stub')) => rename($file, determineSeparator('./database/migrations/create_' . titleSnake($packageSlugWithoutPrefix) . '_table.php.stub')),
        str_contains($file, determineSeparator('config/slotgen-gatesofolympus.php')) => rename($file, determineSeparator('./config/' . $packageSlugWithoutPrefix . '.php')),
        str_contains($file, determineSeparator('resources/lang/en/slotgen-gatesofolympus.php')) => rename($file, determineSeparator('./resources/lang/en/' . $packageSlugWithoutPrefix . '.php')),
...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i ' . basename(__FILE__) . ' | findstr /r /i /M /F:/ ":author :vendor :package Slotgen slotgen-gatesofolympus migration_table_name vendor_name vendor_slug author@domain.com"'));
}
...
function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('find ./* ./.github/* -name "vendor" -type d -prune \
     ...
     -o -type f -print0 | xargs -0 grep -E -r -l -i ":author|:vendor|:package|Slotgen|slotgen-gatesofolympus|migration_table_name|vendor_name|vendor_slug|author@domain.com"'));
}
